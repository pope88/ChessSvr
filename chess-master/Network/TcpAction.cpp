...
void TcpActionClose::run( TcpServer * ts )
{
	if(pend)
		ts->pendClose2(id);
	else
...
void TcpActionSend::run( TcpServer * ts )
{
	if(packet->empty())
		return;
	if(gid == 0xFFFFFFFF)
...
void TcpActionBroadcast::run( TcpServer * ts )
{
	if(packet->empty())
		return;
...
void TcpActionMultiSend::run( TcpServer * ts )
{
