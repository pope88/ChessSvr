...
一对:		 2							Card.Value(面值)			2
三张:		 3							Card.Value(面值)			3

三带一:		 31							三张的Card.Value(面值)		4
四张:		 4							Card.Value(面值)			4
...
public:
	Card() : m_nColor(-1), m_nValue(-1) {}
	Card(int nColor, int nValue) : m_nColor(nColor), m_nValue(nValue) {}
	Card (const Card &srcCard)
	{
		if(this == &srcCard)
			return;
	...
	Card& operator = (const Card &srcCard)
	{
		if (this == &srcCard)
			return *this;
		m_nColor = srcCard.m_nColor;
	...
	bool operator == (const Card &srcCard)
	{
...
public:
	...
	void SetBaoValue(const Card& card);		//设置宝牌
    ...
    bool FindBaiArray(bool m_bfirst,CCardsType& cCardsType);             //
