...
%         [OldV] -> LoginTs = OldV#ets_online_actor.login_ts
%     end,
%     Session = lib_seed:rand(1, 1000000),
%     Seq = lib_seed:rand(1, 1000),
%     Actor = #ets_online_actor{rid = ActorRid, session = Session, seq = Seq, login_ts = LoginTs},
%     ets:insert(?ETS_ONLINE_ACTOR, Actor),
%     {Session, Seq}.
% delete_actor(ActorRid) ->
%     %需要上报微讯玩家在线
%     case ets:lookup(?ETS_ONLINE_ACTOR, ActorRid) of
%         [] -> OnlineSeconds = 7200;%默认在线小时嘿嘿
%         [V] -> 
%             LoginTs = V#ets_online_actor.login_ts,
%             LastMsgTs = actor:get(#ets_actor.last_msg_ts, ActorRid),
%             OnlineSeconds = LastMsgTs - LoginTs
%     end,
%     oss_wx_api:log_logout({ActorRid, OnlineSeconds}),
%     csmsg:msg_queue_clean(ActorRid),
%     ets:delete(?ETS_ONLINE_ACTOR, ActorRid).